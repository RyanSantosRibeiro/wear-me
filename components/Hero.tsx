...
import { WearmeWidget } from "./wearme-widget";
...
export const HeroSection = () => {
  ...
  return (
    <section className="relative pt-32 pb-20 px-6 overflow-hidden min-h-screen flex items-center h-auto max-w-screen flex-col gap-[20vh]">
      {/* Background Decor */}
      ...
      <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-12 lg:gap-20 items-center h-auto max-w-full min-h-[70vh]">
        ...
        <div className={`relative w-full max-w-[300px] w-full mx-auto lg:max-w-[none] h-[80%] flex items-center justify-center ${!out ? `animate-${selectedCategory}` : 'animate-out'}`}>
          {/* 3D Floating Effect Container */}
          <WearmeWidget productImage={categoryImage["casual"]} />

          {/* Floating Elements for Decoration */}
          <div id="wearme-floating-1" className="wearme-floating absolute -top-12 -right-6 bg-white p-4 rounded-2xl shadow-xl shadow-black/5 animate-float delay-100 hidden lg:block">
      ...
      {/* Hero - Experience */}
      <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-12 lg:gap-20 items-center h-auto max-w-full">
        {/* Left Column: Text Content */}
        <div className="space-y-8 text-center lg:text-left">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-white rounded-full border border-primary/10 shadow-sm animate-bounce-subtle mx-auto lg:mx-0">
            <span className="relative flex h-2 w-2">
          ...
          <div className="flex items-center justify-center lg:justify-start gap-2 xl:gap-8 opacity-60 transition-all duration-500">
            {/* button sport */}
            <Button onClick={() => handleCategoryChange("sport")} className={`relative overflow-hidden h-14 px-2 xl:px-8 border-2 border-green-300 hover:border-green-500 !text-green-500 !bg-green-50 ${selectedCategory === "sport" ? "border-green-500 !bg-green-200 hero-active" : ""}`}>
            ...
            {/* button vestido */}
            <Button onClick={() => handleCategoryChange("casual")} className={`relative overflow-hidden h-14 px-2 xl:px-8 border-2 border-rose-300 hover:border-rose-500 !text-rose-500 !bg-rose-50 ${selectedCategory === "casual" ? "border-rose-500 !bg-rose-200 hero-active" : ""}`}>
            ...
            {/* button acessorios */}
            <Button onClick={() => handleCategoryChange("accessories")} className={`relative overflow-hidden h-14 px-2 xl:px-8 border-2 border-amber-300 hover:border-amber-500 !text-amber-500 !bg-amber-50 ${selectedCategory === "accessories" ? "border-amber-500 !bg-amber-200 hero-active" : ""}`}>
        ...
        <div className={`relative w-full max-w-lg mx-auto lg:max-w-none h-auto ${!out ? `animate-${selectedCategory}` : 'animate-out'}`}>
          {/* 3D Floating Effect Container */}
          <div id="wearme-container" className="wearme-container relative bg-white border border-border rounded-[2.5rem] shadow-2xl shadow-primary/10 overflow-hidden min-h-[70vh] md:min-h-[55vh] flex items-center justify-center transform hover:rotate-y-1 transition-transform duration-500">
            ...
            <div id="wearme-background" className={`absolute inset-0 flex flex-col transition-all duration-500 h-fit ${isModalOpen ? 'opacity-30 scale-95 blur-sm' : 'opacity-100 scale-100'}`}>
              ...
              {/* Mockup Content */}
              <div className="flex-1 p-6 flex flex-col lg:flex-row gap-6 items-center lg:items-start overflow-hidden h-auto max-lg:pt-2">
                {/* Product Image */}
                <div className="w-full lg:w-1/2 lg:aspect-[3/4] bg-gray-100 rounded-2xl relative overflow-hidden shadow-inner group min-h-[300px] max-h-[300px] lg:max-h-[400px]">
